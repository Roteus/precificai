import { RawMaterial, UtilityCost, CostBreakdown, Sector } from './types.js';
 ...
 */
export function calculateRawMaterialCost(material: RawMaterial): number {
  try {
    // Converte a quantidade usada para a unidade do total
    const quantityInTotalUnit = convertUnit(
    ...
    if (material.totalQuantity === 0) return 0;
 ...
 */
export function calculateTotalRawMaterialsCost(materials: RawMaterial[]): number {
...
export function calculateUtilityCost(
  ...
  if (!utility || !utility.enabled) return 0;
...
export function calculateUtilitiesCosts(utilities: {
  ...
  water?: UtilityCost;
  gas?: UtilityCost;
}): { energy: number; water: number; gas: number } {
  return {
 ...
 */
export function calculateFinalPrice(totalCost: number, profitMargin: number): number {
 ...
 */
export function getSuggestedMargin(sector: Sector): number {
 ...
 * Calcula o markup
 */
export function calculateMarkup(finalPrice: number, totalCost: number): number {
